...
function Node(config) {
  ...
  this.stopping = false;
  // Set by scaffold/start.js's exit handlers before calling stop(), so
  // services (e.g. bitcoind's spawned pirated process) can log *why*
  // shutdown was triggered - an operator SIGINT vs. an uncaught
  // exception in bitcore itself are very different signals to see in
  // the logs when diagnosing an unexpected stop.
...
Node.prototype.stop = function(callback) {
  log.info('Beginning shutdown (' + (this.stopReason || 'reason not specified') + ')');
  var self = this;
