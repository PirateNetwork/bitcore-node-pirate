...
// Human-readable duration, used in pirated lifecycle logging (startup
// time, uptime before an unexpected exit) - most waits here span
// seconds to tens of minutes, so plain ms would be hard to read.
...
Bitcoin.DEFAULT_START_MAX_WAIT = 1800000; // 30 minutes - large chains (millions of blocks)
                                            // can take a long time to finish loading the
                                            // block index before the RPC server responds
...
Bitcoin.prototype._initDefaults = function(options) {
  ...
  this.startMaxWait = options.startMaxWait || Bitcoin.DEFAULT_START_MAX_WAIT;
...
Bitcoin.prototype._initCaches = function() {
  ...
  this.utxosCache = new LRUCache({max: 50000});
  this.txidsCache = new LRUCache({max: 50000});
  this.balanceCache = new LRUCache({max: 50000});
  this.summaryCache = new LRUCache({max: 50000});
  this.blockOverviewCache = new LRUCache({max: 144});
  this.transactionDetailedCache = new LRUCache({max: 100000});

  // caches valid indefinitely
  this.transactionCache = new LRUCache({max: 100000});
  this.rawTransactionCache = new LRUCache({max: 50000});
  this.blockCache = new LRUCache({max: 144});
  this.rawBlockCache = new LRUCache({max: 72});
  this.blockHeaderCache = new LRUCache({max: 288});
  this.zmqKnownTransactions = new LRUCache({max: 5000});
  this.zmqKnownBlocks = new LRUCache({max: 50});
  this.lastTip = 0;
...
Bitcoin.prototype._loadSpawnConfiguration = function(node) {
  ...
  $.checkArgument(this.options.spawn, 'Please specify "spawn" in pirated config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in pirated config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in pirated config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './PIRATE.conf');

  log.info('Using pirate config file:', configPath);
...
Bitcoin.prototype._checkConfigIndexes = function(spawnConfig, node) {
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to pirated from bitcore. ' +
      'Please add "server=1" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubrawtx,
    '"zmqpubrawtx" option is required to get event updates from pirated. ' +
      'Please add "zmqpubrawtx=tcp://127.0.0.1:<port>" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubhashblock,
    '"zmqpubhashblock" option is required to get event updates from pirated. ' +
  ...
  $.checkState(
    ...
    '"zmqpubrawtx" and "zmqpubhashblock" are expected to the same host and port in pirate.conf'
  );

  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that pirated is undergoing a reindex. ' +
...
Bitcoin.prototype._resetCaches = function() {
  this.transactionDetailedCache.clear();
  this.utxosCache.clear();
  this.txidsCache.clear();
  this.balanceCache.clear();
  this.summaryCache.clear();
  this.blockOverviewCache.clear();
};
...
Bitcoin.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === bitcore.Networks.testnet) {
    networkPath = 'testnet3/PIRATE.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/PIRATE.conf';
    }
  }
  return networkPath;
};
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('Pirate Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
// Runs the ZMQ receive loop in the background. The `zeromq` package (this
// fork's replacement for the abandoned, Node-24-incompatible `zmq`
// package) delivers messages via async iteration on the socket itself
// rather than a 'message' event, so this loop is started (not awaited)
// and runs for the lifetime of the socket.
Bitcoin.prototype._subscribeZmqEvents = function(node) {
  ...
  (async function() {
    try {
      for await (const [topic, message] of node.zmqSubSocket) {
      ...
      if (!node.zmqSubSocket.closed) {
        log.error('Error in ZMQ message loop:', err);
      }
    }
  })();
};
...
Bitcoin.prototype._initZmqSubSocket = function(node, zmqUrl) {
  node.zmqSubSocket = new zmq.Subscriber();
  node.zmqSubSocket.connect(zmqUrl);

  // `events` is the modern replacement for the old binding's
  // .monitor()/'connect'/'connect_delay'/'disconnect'/'monitor_error'
  // events - it's a first-class async-iterable on the socket, so there's
  // no separate monitoring socket that can itself error out and need
  // restarting the way the old binding's monitor_error handler did.
  (async function() {
    try {
      for await (const event of node.zmqSubSocket.events) {
        if (event.type === 'connect') {
          log.info('ZMQ connected to:', event.address);
        } else if (event.type === 'connect:delay' || event.type === 'connect:retry') {
          log.warn('ZMQ connection delay:', event.address);
        } else if (event.type === 'disconnect' || event.type === 'close') {
          log.warn('ZMQ disconnect:', event.address);
        }
      }
    } catch (err) {
      if (!node.zmqSubSocket.closed) {
        log.error('Error in ZMQ event monitoring:', err);
      }
    }
  })();
};
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('Pirate Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
...
// Total readiness-poll attempts needed to cover startMaxWait at
// startRetryInterval spacing - keeps the poll cadence (how often to ask
// "ready yet?") and the total wait ceiling (how long to keep asking)
// independently configurable instead of a single fixed retry count.
Bitcoin.prototype._startRetryTimes = function() {
  return Math.ceil(this.startMaxWait / this.startRetryInterval);
};
...
Bitcoin.prototype._stopSpawnedBitcoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/pirated.pid';
  ...
  function stopProcess() {
    fs.readFile(pidPath, 'utf8', function(err, pid) {
      ...
      try {
        log.warn('Found leftover pirated process (pid ' + pid + ') from a previous run - stopping it before starting fresh');
        self._process.kill(pid, 'SIGINT');
      } catch(err) {
        if (err && err.code === 'ESRCH') {
          log.warn('Leftover pirated process (pid ' + pid + ') is already gone, continuing');
          return callback(null);
        } else if(err) {
          return callback(err);
        }
...
Bitcoin.prototype._spawnChildProcess = function(callback) {
  ...
  self._stopSpawnedBitcoin(function(err) {
    ...
    var spawnStartedAt = Date.now();
    log.info('Starting pirated: ' + self.spawn.exec + ' ' + options.join(' '));
    self.spawn.process = spawn(self.spawn.exec, options, {stdio: 'inherit'});

    self.spawn.process.on('error', function(err) {
      log.error('Failed to spawn pirated (bitcore error, check the configured exec path): ' + err.message);
      self.emit('error', err);
    });

    self.spawn.process.once('exit', function(code) {
      var uptime = formatDuration(Date.now() - spawnStartedAt);
      if (!self.node.stopping) {
        log.warn('pirated exited unexpectedly (pirated error) with code ' + code + ' after running for ' + uptime);
        log.warn('Restarting pirated in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('pirated restarted');
          });
        }, self.spawnRestartTime);
      } else {
        log.info('pirated exited (code ' + code + ') after running for ' + uptime +
          ' as part of a bitcore-initiated shutdown - ' + (self.node.stopReason || 'reason not specified'));
      }
    });
    ...
    async.retry({times: self._startRetryTimes(), interval: self.startRetryInterval}, function(done) {
      if (self.node.stopping) {
        exitShutdown = true;
        return done();
      }
      ...
      attempt++;
      // Only log a fraction of the poll attempts by default (every 6th,
      // ~30s apart at the default 5s interval) so a long block-index
      // load doesn't flood the log with one line per poll, while still
      // showing visible progress during a wait that can span tens of
      // minutes on a multi-million-block chain.
      if (attempt > 1 && attempt % 6 === 0) {
        log.info('Still waiting for pirated to become ready (attempt ' + attempt + '/' + self._startRetryTimes() +
          ', ' + formatDuration(Date.now() - spawnStartedAt) + ' elapsed)');
      }
      ...
      if (err) {
        log.error('Gave up waiting for pirated to become ready after ' + formatDuration(Date.now() - spawnStartedAt) +
          ' (' + attempt + ' attempts) - last error: ' + err.message);
        return callback(err);
      }
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn pirated.'));
      }

      log.info('pirated RPC became ready after ' + formatDuration(Date.now() - spawnStartedAt) +
...
Bitcoin.prototype._connectProcess = function(config, callback) {
  ...
  async.retry({times: self._startRetryTimes(), interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to pirated.'));
    }
...
Bitcoin.prototype.start = function(callback) {
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('Pirate configuration options "spawn" or "connect" are expected'));
    }
...
Bitcoin.prototype.getBlockHashesByTimestamp = function(high, low, callback) {
  var self = this;
  self.client.getBlockHashes(high, low, {noOrphans: true}, function(err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
    }
    callback(null, response.result);
  ...
  if (tx) {
    ...
    self._tryAllClients(function(client, done) {
      client.getRawTransaction(txid, 1, function(err, response) {
        ...
        // Ironwood (v6) START - result.ironwood is TxIronwoodBundleToJSON's
        // output (rpc/rawtransaction.cpp): {actions[], valueBalance,
        // valueBalanceZat, flags, anchor, proof, bindingSig}, or an empty
        // object with zero actions or on any internal extraction error -
        // only attach it when there's real data to show.
        if (tx.version === 6 && result.ironwood && result.ironwood.actions && result.ironwood.actions.length > 0) {
          tx.ironwood = result.ironwood;
        }
        // Ironwood END
...
// bitcoind-rpc's callspec (node_modules/bitcoind-rpc/lib/index.js) only
// generates a client method for RPCs it explicitly lists, and
// getnetworkhashps/getblocksubsidy aren't in that list - call them the
// same way its own generated methods do internally (JSON-RPC over HTTP
// with HTTP Basic auth) rather than patching the vendored client.
Bitcoin.prototype._rawRpcCall = function(method, params, callback) {
  ...
  var auth = new Buffer(client.user + ':' + client.pass).toString('base64');
  var options = {
    host: client.host,
    path: '/',
    method: 'POST',
  ...
  var req = client.protocol.request(options, function(res) {
    ...
    res.on('end', function() {
      ...
      try {
        ...
        return callback(new Error('Pirate JSON-RPC: Error Parsing JSON: ' + e.message));
      }
      callback(parsed.error, parsed.result);
    });
  });
  req.on('error', function(err) {
    callback(new Error('Pirate JSON-RPC: Request Error: ' + err.message));
  });
...
// getnetworkhashps is explicitly deprecated in favor of getnetworksolps
// for this Equihash-based chain (both call the same underlying
// GetNetworkHashPS in TreasureChest's src/rpc/mining.cpp - it's a naming
// change only), and this app displays it as "sols/s" to match.
Bitcoin.prototype.getNetworkSolps = function(callback) {
  var self = this;
  this._rawRpcCall('getnetworksolps', [], function(err, result) {
    if (err) {
      return callback(self._wrapRPCError(err));
    }
...
Bitcoin.prototype.getBlockSubsidy = function(height, callback) {
  var self = this;
  this._rawRpcCall('getblocksubsidy', height != null ? [height] : [], function(err, result) {
    if (err) {
      return callback(self._wrapRPCError(err));
    }
    callback(null, result);
...
Bitcoin.prototype.getTxOutSetInfo = function(callback) {
  var self = this;
...
// Aggregates the RPCs behind the "Stats" page into one call: current
// height/difficulty (getinfo, including the extra per-era halving
// field Komodo assetchains like this one report - see TreasureChest's
// src/rpc/misc.cpp getinfo handler), circulating supply
// (gettxoutsetinfo), network solutions/sec (getnetworksolps), and the
// current/next block subsidy (getblocksubsidy). The next halving's ETA
// is estimated from the chain's own recent average block time (there's
// no single RPC-exposed "target block time" for an assetchain's actual
// runtime config), by diffing the tip's timestamp against a block
// ~2000 blocks back.
Bitcoin.prototype.getChainStats = function(callback) {
  var self = this;

  self.client.getInfo(function(err, infoResponse) {
    if (err) {
      return callback(self._wrapRPCError(err));
    }
    var info = infoResponse.result;
    var height = info.blocks;
    var sampleBack = Math.min(2000, height);

    async.parallel({
      txOutSet: function(next) {
        self.getTxOutSetInfo(next);
      },
      networkSolps: function(next) {
        self.getNetworkSolps(next);
      },
      currentSubsidy: function(next) {
        self.getBlockSubsidy(height, next);
      },
      nextSubsidy: function(next) {
        self.getBlockSubsidy(height + 1, next);
      },
      tipBlock: function(next) {
        self.client.getBlock(self.tiphash, next);
      },
      pastBlock: function(next) {
        if (sampleBack <= 0) {
          return next(null, null);
        }
        self.client.getBlockHash(height - sampleBack, function(err, response) {
          if (err) {
            return next(err);
          }
          self.client.getBlock(response.result, next);
        });
      }
      ...
      var halvingIntervals = (info.halving || '')
        .split(',')
        .filter(Boolean)
        .map(Number);
      var currentEraInterval = halvingIntervals[halvingIntervals.length - 1];
      var nextHalvingHeight = null;
      var nextHalvingEta = null;
      if (currentEraInterval) {
        nextHalvingHeight = (Math.floor(height / currentEraInterval) + 1) * currentEraInterval;

        var tipTime = results.tipBlock && results.tipBlock.result && results.tipBlock.result.time;
        var pastTime = results.pastBlock && results.pastBlock.result && results.pastBlock.result.time;
        if (tipTime && pastTime && sampleBack > 0) {
          var avgBlockSeconds = (tipTime - pastTime) / sampleBack;
          if (avgBlockSeconds > 0) {
            var secondsToHalving = (nextHalvingHeight - height) * avgBlockSeconds;
            nextHalvingEta = new Date((tipTime + secondsToHalving) * 1000).toISOString();
          }
        }
      }
      ...
      callback(null, {
        ...
        supply: results.txOutSet && results.txOutSet.total_amount,
        networkSolps: results.networkSolps,
        reward: results.currentSubsidy && results.currentSubsidy.miner,
        nextReward: results.nextSubsidy && results.nextSubsidy.miner,
        nextHalvingHeight: nextHalvingHeight,
        nextHalvingEta: nextHalvingEta
      });
    });
  });
};
...
Bitcoin.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    log.info('Stopping pirated (' + (this.node.stopReason || 'reason not specified') + ')');
    var exited = false;
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('pirated spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('pirated process did not exit'));
      }
