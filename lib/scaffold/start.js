...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for pirated is necessary for this upgrade with the "reindex=1" PIRATE.conf option. \n' +
                 'There are changes necessary in both PIRATE.conf and bitcore-node.json. \n\n' +
        ...
        servicesConfig: {
          bitcoind: {
            spawn: {
              ...
              exec: path.resolve(__dirname, '../../bin/pirated')
            }
          }
        }
      };
...
function start(options) {
  ...
  node.start(function(err) {
    if(err) {
      ...
      node.stopReason = 'bitcore error: failed to start services: ' + err.message;
...
function exitHandler(options, _process, node, err) {
  if (err) {
    ...
    node.stopReason = 'bitcore error: uncaught exception: ' + err.message;
    node.stop(function(err) {
      if(err) {
        log.error('Failed to stop services: ' + err);
      }
  ...
  if (options.sigint) {
    if (!shuttingDown) {
      shuttingDown = true;
      node.stopReason = 'operator requested shutdown (SIGINT)';
